#!/bin/zsh
# ==============================================================================
# Jamf Deployment Script: Deploy the Claude Code repo-tracker DEBUG hook
# Target Path: /usr/local/bin/claude_debug.py
#
# Runs as root via Jamf policy. Writes the DEBUG twin of the PostToolUse hook to
# a stable system path for on-machine diagnosis of why metrics aren't reaching
# Coralogix (TLS proxy, auth, endpoint, routing). Unlike the production hook it
# logs every decision point and the FULL HTTP response.
#
# The Python below is kept byte-for-byte in sync with
# claude-code/hooks/debug/claude_debug.py.
#
# This file carries NO secrets. The API key is read only from CX_HOOK_API_KEY
# (or an OTLP Bearer header) in the hook subprocess environment.
# ==============================================================================
...
TARGET_FILE="${TARGET_DIR}/claude_debug.py"
...
echo "Deployed Claude Code repo-tracker DEBUG hook to ${TARGET_FILE}"
exit 0
