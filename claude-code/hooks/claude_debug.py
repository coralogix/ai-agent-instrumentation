...
Output goes to BOTH the console (stderr) and a debug log file so the developer
...
# ---------------------------------------------------------------------------
# Logging setup — console (stderr) + file
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    if not raw.strip():
        logger.error("ABORT: stdin was EMPTY. Pipe a JSON event into the script "
                     "(see the PowerShell one-liner).")
        _flush_and_report()
        return
    try:
