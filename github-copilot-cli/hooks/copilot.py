...
"""GitHub Copilot CLI postToolUse hook that tracks repository names per session.

Emits an OTLP gauge metric copilot_cli_session_repo_info with labels
...
Copilot pipes the hook event JSON to stdin. The payload uses camelCase
fields when the hook is registered under a camelCase event name
(``postToolUse``) and snake_case fields under the PascalCase name
(``PostToolUse``); this hook reads both so either registration works.
...
# Configuration
#
# Reuses the same CX_* credentials the other agents source from .env. Exits
# silently if endpoint or key are unset.
# ---------------------------------------------------------------------------
...
# Copilot does not carry the user identity in the hook payload; allow an
# explicit override and otherwise fall back to the local git identity.
USER_EMAIL_OVERRIDE = os.environ.get("CX_HOOK_USER_EMAIL", "")

# tool argument keys that may carry a filesystem path. cwd is always present
# and is the anchor.
...
def extract_paths(event: dict) -> list[str]:
    ...
    # camelCase (toolArgs) or PascalCase/VS Code (tool_input)
...
def build_otlp_protobuf(session_id: str, repo_name: str, user_email: str) -> bytes:
    ...
    metric = _encode_string(1, "copilot_cli_session_repo_info")
    ...
    resource = _field_bytes(1, _encode_kv("service.name", "copilot-cli-hook"))
    if APPLICATION_NAME:
