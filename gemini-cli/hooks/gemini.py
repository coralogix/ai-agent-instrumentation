...
"""Gemini CLI AfterTool hook that tracks repository names per session.

Emits an OTLP gauge metric gemini_cli_session_repo_info with labels
...
Gemini CLI pipes the hook event JSON to stdin and expects a JSON object on
stdout (anything else is treated as malformed) — so this hook prints "{}"
and keeps all diagnostics off stdout. session_id matches the session id on
Gemini CLI's native OTLP telemetry, so the two join downstream on that key.
...
# Configuration
#
# Reuses the same CX_* credentials Gemini CLI's telemetry sources from .env.
# Exits silently if endpoint or key are unset.
# ---------------------------------------------------------------------------
...
# Gemini does not carry the user identity in the hook payload; allow an
# explicit override and otherwise fall back to the local git identity.
USER_EMAIL_OVERRIDE = os.environ.get("CX_HOOK_USER_EMAIL", "")

# tool_input keys that may carry a filesystem path across Gemini's tool set
# (run_shell_command, read_file, write_file, replace, ...). cwd is always
# present and is the anchor.
PATH_KEYS = ("file_path", "path", "absolute_path", "filename", "filepath", "target_file")
...
def build_otlp_protobuf(session_id: str, repo_name: str, user_email: str) -> bytes:
    ...
    metric = _encode_string(1, "gemini_cli_session_repo_info")
    ...
    resource = _field_bytes(1, _encode_kv("service.name", "gemini-cli-hook"))
    if APPLICATION_NAME:
...
if __name__ == "__main__":
    try:
        ...
        pass
    # Gemini CLI requires a JSON object on stdout; emit an empty (no-op) result.
    sys.stdout.write("{}")
