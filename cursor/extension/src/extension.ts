...
// The command registered in hooks.json — the runtime wrapper's identity for
// install-detection, merge/removal, and uninstall.
...
function installPythonDeps(output: vscode.OutputChannel): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    function runPip(cmd: string, prefixArgs: string[], extraArgs: string[], onFail: () => void): void {
      ...
      // spawn() reports a missing interpreter (ENOENT) asynchronously via
      // 'error' rather than throwing — without this handler it would surface
      // as an unhandled event. Fall through to the next candidate instead.
      proc.on('error', () => {
        if (settled) return;
        settled = true;
        onFail();
    ...
    if (IS_WIN) {
      // Try python, then py -3, then python3 — first interpreter that
      // successfully spawns is reused for the install.
