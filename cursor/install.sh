...
#
# Run with --help for usage. Requires Python 3.8+; the installer probes PATH and
# the usual install prefixes for an interpreter that actually runs (see below).
...
# ---------------------------------------------------------------------------
# Python (runs the hook, and edits hooks.json for both install and uninstall)
#
# Two traps make a bare `python3` unusable here:
#
#   1. macOS ships no Python of its own. /usr/bin/python3 is a placeholder that
#      satisfies `command -v` and then fails when executed, unless Apple's
#      developer command line tools are installed — so a name-only check passes
#      on exactly the machines that cannot run the hook.
#   2. Cursor is launched from the GUI, so its hooks run with a bare-bones PATH
#      that usually lacks /opt/homebrew/bin. A python3 that works in a terminal
#      can be invisible to the hook at runtime.
#
# So: probe candidates by actually RUNNING them, and pin the absolute path of
# the winner into the wrapper instead of letting it resolve `python3` itself.
# ---------------------------------------------------------------------------
...
# A python3 on PATH wins when it works — that is the one the user chose (pyenv,
# uv, asdf, a distro package).
PY_ON_PATH="$(command -v python3 2>/dev/null || true)"

# One ordered, de-duplicated list: probed here, reported verbatim on failure.
PY_TRIED=()
[[ -n "$PY_ON_PATH" ]] && PY_TRIED+=("$PY_ON_PATH")
for candidate in "${PY_CANDIDATES[@]}"; do
  [[ "$candidate" == "$PY_ON_PATH" ]] || PY_TRIED+=("$candidate")
done

PY_BIN=""
for candidate in "${PY_TRIED[@]}"; do
...
if [[ -z "$PY_BIN" ]]; then
  {
    echo "Error: no working python3 found — the hook cannot run."
    echo
    echo "Tried, in order (each must run '-c pass' successfully):"
    for candidate in "${PY_TRIED[@]}"; do
      if [[ ! -e "$candidate" ]]; then
        ...
        # Surface the interpreter's own first line of stderr: on a Mac with no
        # developer tools that is where the /usr/bin/python3 placeholder explains
        # itself, which is far more useful than "not found".
        # `|| true`: the probe is expected to fail, and this script runs under
        # `set -e -o pipefail` — without it the diagnostic kills itself.
        reason="$("$candidate" -c 'pass' 2>&1 | head -1 || true)"
        reason="${reason:-exits non-zero}"
      fi
      printf '  %-56s %s\n' "$candidate" "$reason"
    done
    echo
    echo "macOS has not shipped a Python since 12.3, and /usr/bin/python3 is only"
    echo "a placeholder for Apple's developer command line tools. Install a real"
    echo "interpreter, then re-run this installer:"
    echo
    echo "  brew install python                      (recommended)"
    echo "  https://www.python.org/downloads/macos/  (signed .pkg, no package manager)"
    echo
    echo "Already installed somewhere unusual? Add its path to PY_CANDIDATES in"
    echo "this script, or put it on your PATH."
  } >&2
  exit 1
fi
...
if $UNINSTALL; then
  ...
  "$PY_BIN" - "$HOOKS_JSON" "$WRAPPER_SH" <<'PYEOF'
import json, sys
...
#
# The interpreter is pinned to an absolute path on purpose: Cursor launches hooks
# with a bare-bones PATH in which macOS's non-functional /usr/bin/python3
# placeholder is often the only python3, so resolving it here would be a lottery.
PY_BIN="$PY_BIN"
EOF
cat >> "$WRAPPER_SH" <<'EOF'
...
# Interpreter went missing since install (Python removed, Homebrew pruned):
# exit quietly. A hook must never break the Cursor session.
[[ -x "$PY_BIN" ]] || exit 0

exec "$PY_BIN" "$HOOK_DIR/coralogix_hook.py"
EOF
...
PIP=("$PY_BIN" -m pip install --quiet --user --no-warn-script-location)
...
if ! "${PIP[@]}" "${PACKAGES[@]}" >/dev/null 2>&1 \
  ...
  echo "  $PY_BIN -m pip install --user ${PACKAGES[*]}" >&2
else
