...
# ErrorActionPreference is Stop; PS 5.1 does not. Disable it so both hosts
# behave the same and $LASTEXITCODE checks below stay meaningful.
...
# Run a Python snippet with extra argv. PS 5.1 ConvertTo-Json unwraps
# single-element arrays into objects, which would corrupt hooks.json, so every
# JSON read/write goes through Python instead — it is a hard dependency of the
# hook anyway. Passing paths as argv also sidesteps backslash escaping.
...
# Restrict the credentials file (the NTFS equivalent of chmod 600): drop
# inherited rules, then grant only the profile owner and the installing
# account. Both are granted because an MDM runs this as SYSTEM against another
# user's profile - granting SYSTEM alone would lock the hook out of its own
# credentials.
try {
    $grantees = @()
...
# Loads coralogix_hook.env into the process environment and pipes Cursor's
# stdin JSON payload through to hook.py.
...
# 5. Write the .cmd shim registered in hooks.json. Cursor on Windows executes
#    hook commands through the shell; a .cmd entry point is what reliably
#    launches PowerShell with stdin attached.
$WrapperCmdContent = @'
@echo off
